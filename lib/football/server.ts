...
async function fetchDate(date:string){return (await providerGet("/fixtures",{date,timezone:"UTC"})).map(normalize);}
async function fetchLive(){return (await providerGet("/fixtures",{live:"all"})).map(normalize);}

export async function getFeaturedMatches(){
  const now=Date.now(),cached=await getCached();
  const today=new Date(now).toISOString().slice(0,10),tomorrow=new Date(now+86400000).toISOString().slice(0,10),horizon=now+10*3600000;
...
export async function getMatchById(fixtureId:string,forceRefresh=false){
  const cached=await getCached([fixtureId]),existing=cached.get(fixtureId),ttl=existing?.match.status.live?45000:300000;
...
export async function searchMatches(query:string){
  ...
  const teams=await providerGet("/teams",{search:clean});
  const teamIds=Array.from(new Set(teams.map((item:any)=>Number(item?.team?.id)).filter(Number.isFinite))).slice(0,3);
...
export async function getAuthenticatedAccessToken(){const cookieStore=await cookies();return cookieStore.get("matchup-access-token")?.value||null;}
