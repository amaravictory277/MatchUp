...
type AdminProfile = { id: string; display_name: string | null; username: string };
...
async function authUser(accessToken: string) {
  ...
  const response = await fetch(`${SUPABASE_URL}/auth/v1/user`, { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` }, cache: 'no-store' });
...
async function refreshAccessToken(refreshToken: string) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) return null;
  const response = await fetch(`${SUPABASE_URL}/auth/v1/token?grant_type=refresh_token`, { method: 'POST', headers: { apikey: SUPABASE_ANON_KEY, 'content-type': 'application/json' }, body: JSON.stringify({ refresh_token: refreshToken }), cache: 'no-store' });
  if (!response.ok) return null;
...
async function databaseSaysAdmin(accessToken: string) {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/is_matchup_admin`, { method: 'POST', headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}`, 'content-type': 'application/json' }, body: '{}', cache: 'no-store' });
...
export async function requireAdmin() {
  ...
  const response = await fetch(url, { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${session.accessToken}` }, cache: 'no-store' });
  if (!response.ok) return null;
  const profile = (await response.json() as AdminProfile[])[0];
  return profile && profile.id === session.userId ? { ...session, profile } : null;
}
...
async function callAdminRpc<T>(accessToken: string, functionName: string, body: Record<string, unknown> = {}) {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/${functionName}`, { method: 'POST', headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}`, 'content-type': 'application/json' }, body: JSON.stringify(body), cache: 'no-store' });
...
export async function fetchAdminProfiles(accessToken: string, search = '', limit = 50, offset = 0) {
  return callAdminRpc<{ users: AdminUser[]; total: number; activeToday: number }>(accessToken, 'get_admin_users', { p_search: search.trim().slice(0, 80), p_limit: limit, p_offset: offset });
}

export async function fetchAdminOnlineUsers(accessToken: string, userIds: string[]) {
  return callAdminRpc<AdminUser[]>(accessToken, 'get_admin_online_users', { p_user_ids: userIds.slice(0, 100) });
}
