...
export function PersonalizationPage({ profile }: { profile: ProfileData }) {
  ...
  const [chatTextPreview, setChatTextPreview] = useState("normal");
  const [chatAnimationPreview, setChatAnimationPreview] = useState("normal");
  const [onlinePreview, setOnlinePreview] = useState("standard");
  ...
  const open = (category: (typeof PERSONALIZATION_CATEGORIES)[number]) => {
    ...
    setChatTextPreview(preferences.selected_chat_text);
    setChatAnimationPreview(preferences.selected_chat_animation);
    setOnlinePreview(preferences.selected_online_effect);
  ...
  const apply = async () => {
    ...
    if (active.key === "chat") {
      await setPreference("selected_chat_bubble", preview);
      await setPreference("selected_chat_text", chatTextPreview);
      await setPreference("selected_chat_animation", chatAnimationPreview);
    } else if (active.key === "ready") {
      await setPreference("selected_ready_effect", preview);
      await setPreference("selected_online_effect", onlinePreview);
    } else {
    ...
    {active ? <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/70 p-0 sm:items-center sm:p-5">
      <section className="max-h-[92dvh] w-full max-w-2xl overflow-y-auto rounded-t-[28px] border border-[#214a78] bg-[#071426] p-5 shadow-2xl sm:rounded-[28px]">
        ...
        <div className={`mt-5 rounded-[24px] border border-[#18365f] bg-[#061221] p-5 ${categoryKey === "theme" ? "min-h-56" : ""}`}>
          {categoryKey === "name" ? <div className="grid min-h-28 place-items-center"><span className={"matchup-name-effect matchup-name-effect--" + selected + " text-4xl font-black"}>{profile.displayName}</span></div> :
           categoryKey === "frame" ? <div className="grid min-h-28 place-items-center"><span className={"matchup-profile-frame matchup-profile-frame--" + selected + " size-28"}><span className="matchup-profile-frame__media">{profile.avatar?<img src={profile.avatar} alt={profile.displayName} className="size-full object-cover"/>:<UserRound size={28} className="text-[#70c1ff]"/>}</span><span className="matchup-profile-frame__ring"/></span></div> :
           categoryKey === "theme" ? <div className={`matchup-theme-preview matchup-theme-preview--${selected}`}><span className={"matchup-profile-frame matchup-profile-frame--" + preferences.selected_profile_frame + " size-20"}><span className="matchup-profile-frame__media">{profile.avatar?<img src={profile.avatar} alt={profile.displayName} className="size-full object-cover"/>:<UserRound size={24} className="text-[#70c1ff]"/>}</span><span className="matchup-profile-frame__ring"/></span><div><span className={"matchup-name-effect matchup-name-effect--" + preferences.selected_name_effect + " text-xl font-black"}>{profile.displayName}</span><p className="text-xs text-[#7892ac]">{[profile.country,profile.game].filter(Boolean).join(" · ")}</p><p className="mt-2 text-xs text-[#7892ac]">Followers · Following · Profile</p></div></div> :
           categoryKey === "chat" ? <div className="flex justify-end"><div className={"matchup-chat-bubble matchup-chat-bubble--" + selected + " matchup-chat-animation--" + chatAnimationPreview}><span className={"matchup-chat-text--" + chatTextPreview + " text-sm"}>This is a live preview of your outgoing message.</span></div></div> :
           ...
           categoryKey === "player" ? <div className={`matchup-player-card-preview matchup-player-card-preview--${selected}`}><span className={"matchup-profile-frame matchup-profile-frame--" + preferences.selected_profile_frame + " size-16"}><span className="matchup-profile-frame__media">{profile.avatar?<img src={profile.avatar} alt={profile.displayName} className="size-full object-cover"/>:<UserRound size={20} className="text-[#70c1ff]"/>}</span><span className="matchup-profile-frame__ring"/></span><div className="min-w-0"><span className={"matchup-name-effect matchup-name-effect--" + preferences.selected_name_effect + " truncate text-lg font-black"}>{profile.displayName}</span><p className="text-xs opacity-70">{[profile.country,profile.game,profile.rating ? `OVR ${profile.rating}` : null].filter(Boolean).join(" · ")}</p></div></div> :
        ...
        <div className="mt-5 grid gap-3">{categoryKey === "chat" ? <><div><p className="mb-2 text-[10px] font-black uppercase tracking-[.14em] text-[#70c1ff]">Chat Text</p><div className="grid gap-2 sm:grid-cols-2">{EFFECTS.chatText.map(option => <button key={option.id} type="button" onClick={() => setChatTextPreview(option.id)} className={"rounded-2xl border p-3 text-left " + (chatTextPreview===option.id ? "border-[#2497ff] bg-[#0b3154]" : "border-[#18365f] bg-[#061221]")}><span className="block text-sm font-black text-white">{option.name}</span><span className="mt-1 block text-[11px] text-[#7892ac]">{option.description}</span></button>)}</div></div><div><p className="mb-2 text-[10px] font-black uppercase tracking-[.14em] text-[#70c1ff]">Message Entrance</p><div className="grid gap-2 sm:grid-cols-2">{EFFECTS.animation.map(option => <button key={option.id} type="button" onClick={() => setChatAnimationPreview(option.id)} className={"rounded-2xl border p-3 text-left " + (chatAnimationPreview===option.id ? "border-[#2497ff] bg-[#0b3154]" : "border-[#18365f] bg-[#061221]")}><span className="block text-sm font-black text-white">{option.name}</span><span className="mt-1 block text-[11px] text-[#7892ac]">{option.description}</span></button>)}</div></div></> : categoryKey === "ready" ? <div><p className="mb-2 text-[10px] font-black uppercase tracking-[.14em] text-[#70c1ff]">Online Appearance</p><div className="grid gap-2 sm:grid-cols-2">{EFFECTS.online.map(option => <button key={option.id} type="button" onClick={() => setOnlinePreview(option.id)} className={"rounded-2xl border p-3 text-left " + (onlinePreview===option.id ? "border-[#2497ff] bg-[#0b3154]" : "border-[#18365f] bg-[#061221]")}><span className="block text-sm font-black text-white">{option.name}</span><span className="mt-1 block text-[11px] text-[#7892ac]">{option.description}</span></button>)}</div></div> : null}</div><div className="mt-5 grid gap-2 sm:grid-cols-2">
