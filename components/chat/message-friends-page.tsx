...
export function MessageFriendsPage() {
  ...
  const avatarUrl = (p: Profile) => p.avatar_path ? supabase.storage.from("avatars").getPublicUrl(p.avatar_path).data.publicUrl : null;

  return <main className="min-h-screen bg-[#061120] px-4 pb-10 pt-5 text-white sm:px-6">
    <div className="mx-auto max-w-3xl">
      <header className="sticky top-0 z-20 -mx-4 flex items-center gap-3 border-b border-white/5 bg-[#061120]/95 px-4 py-4 backdrop-blur sm:-mx-6 sm:px-6">
        <button type="button" onClick={() => router.back()} className="icon-button" aria-label="Back"><ArrowLeft size={18} /></button>
        <div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#47a8ff]">Messaging</p><h1 className="text-2xl font-black">Message New Friends</h1></div>
      </header>
      {loading ? <div className="py-16 text-center text-sm text-[#7892ac]">Loading your conversations…</div> : <>
        <section className="pt-7">
          <div className="mb-3 flex items-end justify-between"><div><h2 className="text-lg font-black">Message New Friends</h2><p className="mt-1 text-xs text-[#7892ac]">Friends you have accepted but have not messaged yet.</p></div>{newFriends.length > 5 ? <button type="button" onClick={() => setShowAllFriends(v => !v)} className="rounded-xl px-3 py-2 text-xs font-black text-[#9bd3ff] hover:bg-[#0a2946]">{showAllFriends ? "Show Less" : "See All"}</button> : null}</div>
          {newFriends.length ? <div className="no-scrollbar flex gap-3 overflow-x-auto pb-2">{(showAllFriends ? newFriends : newFriends.slice(0, 5)).map(p => { const avatar = avatarUrl(p); return <div key={p.id} className="min-w-[155px] rounded-2xl border border-[#18365f] bg-[#071426] p-4">
            <div className="grid size-14 place-items-center overflow-hidden rounded-full bg-[#0b3154] text-sm font-black text-[#70c1ff]">{avatar ? <img src={avatar} alt="" className="size-full object-cover" /> : initials(p)}</div>
            <p className="mt-3 truncate text-sm font-black">{nameOf(p)}</p>
            <button type="button" disabled={busy === p.id} onClick={() => void message(p)} className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-xl bg-[#167bd1] px-3 py-2.5 text-xs font-black disabled:opacity-50"><MessageCircle size={14} />{busy === p.id ? "Opening…" : "Message"}</button>
          </div>; })}</div> : <div className="rounded-2xl border border-dashed border-[#18365f] p-6 text-center text-sm text-[#7892ac]">You have no unmessaged friends right now.</div>}
        </section>
        <section className="mt-9">
          <div className="rounded-2xl border border-[#194b7c] bg-[#08182b] px-4 py-3"><h2 className="text-center text-sm font-black tracking-[.14em] text-[#9bd3ff]">MY CHATS</h2></div>
          <div className="mt-3 space-y-2">{(showAllChats ? chats : chats.slice(0, 3)).map(c => <button key={c.id} type="button" onClick={() => router.push(`/leaderboard?group=${c.id}`)} className="flex w-full items-center gap-3 rounded-2xl border border-[#18365f] bg-[#071426] p-4 text-left"><span className="grid size-11 place-items-center rounded-full bg-[#0b3154] text-[#70c1ff]"><MessageCircle size={18} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-black">{c.name}</span><span className="block text-[11px] text-[#7892ac]">{c.kind === "group" ? "Group" : "Conversation"}</span></span><span className="text-[10px] text-[#7892ac]">{c.lastAt ? new Date(c.lastAt).toLocaleDateString() : ""}</span></button>)}{!chats.length ? <p className="rounded-2xl border border-dashed border-[#18365f] p-7 text-center text-sm text-[#7892ac]">No chats yet.</p> : null}</div>
          {chats.length > 3 ? <button type="button" onClick={() => setShowAllChats(v => !v)} className="mt-3 w-full rounded-xl border border-[#245b91] bg-[#0a2946] px-4 py-3 text-sm font-black text-[#9bd3ff]">{showAllChats ? "Show Less" : "See All"}</button> : null}
        </section>
