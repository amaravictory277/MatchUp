...
export function ChatVisualEnhancer() {
  ...
  useEffect(() => {
    ...
    const savePosition = () => {
      ...
      try { sessionStorage.setItem(`${SCROLL_KEY}${activeKey()}`, String(body.scrollTop)); } catch {}
    };
    ...
    const restoreSavedPosition = () => {
      ...
      window.requestAnimationFrame(() => { restoringPosition = false; savePosition(); });
      return true;
    };
    ...
    const ensureNewMessageIndicator = () => {
      ...
      indicator.innerHTML = '<span data-matchup-new-count></span><button type="button">Scroll to newest <span aria-hidden="true">↓</span></button>';
    ...
    const onScroll = () => {
      ...
      if (current < 120 && !loadingOlder) {
        ...
        if (loader && !loader.disabled) { loadingOlder = true; previousHeight = body.scrollHeight; previousTop = current; loader.click(); }
      }
    ...
    const onViewport = () => { window.cancelAnimationFrame(keyboardFrame); keyboardFrame = window.requestAnimationFrame(setKeyboardSafeComposer); };
    ...
    const initialApply = () => {
      ...
      previousIds = new Set(Array.from(body.querySelectorAll<HTMLElement>("[data-message-id]")).map(el => el.dataset.messageId || ""));
      if (savedPosition !== null) {
        restoreSavedPosition();
        window.setTimeout(() => { if (anchor && originalScrollIntoView) anchor.scrollIntoView = originalScrollIntoView; }, 220);
      }
    };
    ...
    initialApply();
    restoreTimer = window.setTimeout(() => { removeOlderButton(); setKeyboardSafeComposer(); if (savedPosition !== null) restoreSavedPosition(); }, 140);
