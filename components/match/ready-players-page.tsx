...
export function ReadyPlayersPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: me }, { data: ps }, { data: reqsIn }, { data: reqsOut }, { data: activeRows }] = await Promise.all([
      ...
      supabase
        .from("match_requests")
        .select("id,requester_id,opponent_id,status,created_at,responded_at,profiles!match_requests_requester_id_fkey(id,display_name,username,avatar_path,ready_player_enabled)")
        .eq("opponent_id", id)
      ...
      supabase
        .from("match_requests")
        .select("id,requester_id,opponent_id,status,created_at,responded_at,profiles!match_requests_opponent_id_fkey(id,display_name,username,avatar_path,ready_player_enabled)")
        .eq("requester_id", id)
        .in("status", ["pending", "accepted"])
        .or(`status.eq.pending,responded_at.gte.${cutoff}`)
        .order("created_at", { ascending: false }),
  ...
  const messageNow = async (friendId: string) => {
    setBusy(`message:${friendId}`);
    setNotice("");
    const { data, error } = await supabase.rpc("get_or_create_private_chat", { p_friend: friendId });
    if (error) setNotice(error.message);
    else if (data) router.push(`/leaderboard?group=${data}`);
    setBusy("");
  };
                  ...
                  <div key={request.id} className="rounded-2xl border border-[#214a78] bg-[#0a2139] p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-[#7892ac]">{accepted ? "You matched — start your private chat." : "wants to play a quick match"}</p>
                      </div>
                    </div>
                    {accepted ? (
