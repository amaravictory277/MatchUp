...
import { ArrowLeft, Camera, Check, ChevronRight, Gamepad2, Globe2, LogOut, Pencil, UserRound, Bell, Sparkles } from "lucide-react";
import { useRouter, useSearchParams } from "next/navigation";
import { createBrowserSupabaseClient } from "../../lib/supabase/client";
import { clearAuthSession } from "../../lib/auth/session";
import { MatchUpVerificationBadge } from "../feeds/matchup-verification-badge";
import { PersonalizationPage } from "../personalization/personalization-page";
import { NameEffect, ProfileFrame, ProfileTheme } from "../personalization/effect-renderers";
...
const currencyByCountry:Record<string,string>={Nigeria:"NGN",Ghana:"GHS",Kenya:"KES","South Africa":"ZAR","United Kingdom":"GBP","United States":"USD"};
...
export function ProfilePage(){
  ...
  const screen=view==="edit" || view==="settings" || view==="personalization" ? view : "profile";
  ...
  useEffect(()=>()=>{if(avatarPreview?.startsWith("blob:"))URL.revokeObjectURL(avatarPreview);if(coverPreview?.startsWith("blob:"))URL.revokeObjectURL(coverPreview)},[avatarPreview,coverPreview]);
  ...
  if(screen==="personalization")return <PersonalizationPage profile={{displayName,avatar,country:profile.country,game:profile.supported_game,rating:profile.player_rating}} />;
  ...
  return <main className="profile-page app-shell">
    ...
    <ProfileTheme><section className="profile-hero surface-card overflow-hidden"><div className="h-28 bg-[radial-gradient(circle_at_50%_0%,rgba(36,151,255,.42),transparent_64%)]"/><div className="-mt-14 px-5 pb-5"><div className="flex items-end justify-between gap-4"><ProfileFrame src={avatar} alt={displayName} className="size-28 border-4 border-[#071426] text-3xl font-black text-[#70c1ff]" fallback={initial}/><button type="button" onClick={()=>openView("edit")} className="rounded-full bg-[linear-gradient(100deg,#126bc0,#2497ff)] px-4 py-2.5 text-xs font-black text-white shadow-[0_8px_22px_rgba(36,151,255,.22)]"><Pencil size={14} className="mr-1 inline"/>Edit Profile</button></div>
      <div className="mt-5 flex items-center gap-2"><h2 className="truncate text-2xl font-black text-white"><NameEffect>{displayName}</NameEffect></h2>{profile.is_verified?<MatchUpVerificationBadge/>:null}</div><p className="mt-1 text-sm text-[#7892ac]">@{profile.username}</p>{profile.bio?.trim()?<p className="mt-3 max-w-2xl text-sm leading-6 text-[#b7c9da]">{profile.bio.trim()}</p>:null}
      <div className="mt-5 grid gap-2 sm:grid-cols-3"><InfoCard icon={UserRound} label="Email" value={email || "Not available"}/>{profile.country ? <InfoCard icon={Globe2} label="Country" value={profile.country}/> : null}{profile.supported_game ? <InfoCard icon={Gamepad2} label="Game" value={profile.supported_game}/> : null}</div>
    </div></section></ProfileTheme>
