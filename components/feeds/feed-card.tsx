...
import { Check, ChevronLeft, ChevronRight, Heart, MessageCircle, MoreHorizontal, Pencil, Play, Send, Trash2, Volume2, VolumeX, X } from "lucide-react";
  ...
  return (
    <article className="surface-card overflow-hidden">
      ...
      <div className="relative aspect-[4/3] w-full touch-pan-y select-none overflow-hidden bg-[#111223]" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp}>
        {post.videoUrl ? <>
          <video ref={videoRef} src={post.videoUrl} poster={post.media[index]} muted playsInline loop preload="metadata" className="absolute inset-0 size-full object-cover" onClick={activateVideo} aria-label={`${post.author.name} video`} />
          <button type="button" onClick={activateVideo} aria-label={soundOn ? "Mute video" : "Play video with sound"} className="absolute bottom-3 right-3 z-10 grid size-10 place-items-center rounded-full border border-white/20 bg-black/55 text-white shadow-lg backdrop-blur-sm">{soundOn ? <Volume2 size={18} /> : <VolumeX size={18} />}</button>
          {!soundOn ? <button type="button" onClick={activateVideo} className="absolute left-1/2 top-1/2 z-10 flex -translate-x-1/2 -translate-y-1/2 items-center gap-2 rounded-full bg-black/55 px-4 py-2.5 text-xs font-bold text-white backdrop-blur-sm"><Play size={15} fill="currentColor" />Tap for sound</button> : null}
