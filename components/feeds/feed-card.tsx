...
import { Check, ChevronLeft, ChevronRight, Heart, MessageCircle, MoreHorizontal, Pause, Pencil, Play, Send, Trash2, X } from "lucide-react";
  ...
  return (
    <article className="surface-card overflow-hidden">
      ...
      <div className="relative aspect-[4/3] w-full touch-pan-y select-none overflow-hidden bg-[#111223]" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp}>
        ...
        {post.hasVideo ? <button type="button" onClick={() => setPlaying((p) => !p)} aria-label={playing ? "Pause" : "Play"} className="absolute left-1/2 top-1/2 grid size-14 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-[3px] border-[#13132b] bg-[linear-gradient(145deg,#8e3cff,#5e1be4)] text-white shadow-[0_0_24px_rgba(122,43,255,.65)] transition hover:scale-105">{playing ? <Pause size={24} /> : <Play size={24} className="ml-0.5" />}</button> : null}
