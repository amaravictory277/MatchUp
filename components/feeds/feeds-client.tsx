...
import { Bell, Bookmark, LogOut, Search, Settings, UserRound, X } from "lucide-react";
  ...
  return (
    <>
      ...
      {composer ? <div className="install-overlay" onClick={(e) => { if (e.target === e.currentTarget) setComposer(null); }}><div className="install-sheet">
        ...
        <textarea value={composerText} onChange={(e) => setComposerText(e.target.value)} autoFocus rows={4} placeholder="What do you want to share?" className="mt-4 w-full resize-none rounded-2xl border border-[#26263d] bg-[#0d0e20] p-3.5 text-sm text-white outline-none placeholder:text-[#6f6d83] focus:border-[#7843ee]" />
