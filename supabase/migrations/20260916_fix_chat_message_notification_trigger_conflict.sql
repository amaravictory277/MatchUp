-- Fix private/group chat message inserts failing inside the notification trigger.
-- notifications_event_key_unique is a partial unique index, so the conflict
-- target must include the same predicate.
  ...
  loop
    ...
    values(
      ...
      jsonb_build_object(
        ...
        'message', case
          when group_kind in ('private','match')
            then actor_name || ' sent you a message'
