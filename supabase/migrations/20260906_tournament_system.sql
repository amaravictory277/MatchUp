...
create policy if not exists "profiles insert own" on public.profiles for insert with check (id = auth.uid());
create policy if not exists "profiles update own" on public.profiles for update using (id = auth.uid()) with check (id = auth.uid());
...
create policy if not exists "built in teams are public" on public.football_teams for select using (true);
create policy if not exists "tournament teams are visible" on public.tournament_teams for select using (exists (select 1 from public.tournaments t where t.id = tournament_id and (t.visibility='public' or t.organizer_id=auth.uid() or public.is_tournament_admin(t.id))));
create policy if not exists "tournament admins manage teams" on public.tournament_teams for all using (public.is_tournament_admin(tournament_id)) with check (public.is_tournament_admin(tournament_id));
create policy if not exists "venues are visible" on public.venues for select using (exists (select 1 from public.tournaments t where t.id = tournament_id and (t.visibility='public' or t.organizer_id=auth.uid() or public.is_tournament_admin(t.id))));
create policy if not exists "admins manage venues" on public.venues for all using (public.is_tournament_admin(tournament_id)) with check (public.is_tournament_admin(tournament_id));
create policy if not exists "admins visible" on public.tournament_admins for select using (public.is_tournament_admin(tournament_id) or user_id = auth.uid());
create policy if not exists "owner manages admins" on public.tournament_admins for all using (exists (select 1 from public.tournaments t where t.id = tournament_id and t.organizer_id = auth.uid())) with check (exists (select 1 from public.tournaments t where t.id = tournament_id and t.organizer_id = auth.uid()));
create policy if not exists "fixtures visible" on public.fixtures for select using (exists (select 1 from public.tournaments t where t.id = tournament_id and (t.visibility='public' or t.organizer_id=auth.uid() or public.is_tournament_admin(t.id))));
create policy if not exists "admins manage fixtures" on public.fixtures for all using (public.is_tournament_admin(tournament_id)) with check (public.is_tournament_admin(tournament_id));
create policy if not exists "roster visible" on public.tournament_players_roster for select using (exists (select 1 from public.tournament_teams tt join public.tournaments t on t.id=tt.tournament_id where tt.id=tournament_team_id and (t.visibility='public' or t.organizer_id=auth.uid() or public.is_tournament_admin(t.id))));
create policy if not exists "admins manage roster" on public.tournament_players_roster for all using (exists (select 1 from public.tournament_teams tt where tt.id=tournament_team_id and public.is_tournament_admin(tt.tournament_id))) with check (exists (select 1 from public.tournament_teams tt where tt.id=tournament_team_id and public.is_tournament_admin(tt.tournament_id)));
create policy if not exists "events visible" on public.match_events for select using (exists (select 1 from public.fixtures f join public.tournaments t on t.id=f.tournament_id where f.id=fixture_id and (t.visibility='public' or t.organizer_id=auth.uid() or public.is_tournament_admin(t.id))));
create policy if not exists "admins manage events" on public.match_events for all using (exists (select 1 from public.fixtures f where f.id=fixture_id and public.is_tournament_admin(f.tournament_id))) with check (exists (select 1 from public.fixtures f where f.id=fixture_id and public.is_tournament_admin(f.tournament_id)));

insert into storage.buckets (id, name, public) values ('team-logos','team-logos',true) on conflict (id) do nothing;
create policy if not exists "public team logos read" on storage.objects for select using (bucket_id='team-logos');
create policy if not exists "authenticated team logo upload" on storage.objects for insert to authenticated with check (bucket_id='team-logos' and (storage.foldername(name))[1] = auth.uid()::text);
create policy if not exists "authenticated team logo update" on storage.objects for update to authenticated using (bucket_id='team-logos' and (storage.foldername(name))[1] = auth.uid()::text);
create policy if not exists "authenticated team logo delete" on storage.objects for delete to authenticated using (bucket_id='team-logos' and (storage.foldername(name))[1] = auth.uid()::text);
